		...
		if($sender instanceof Player){
			...
			if (strtolower($command->getName()) === "f") {
				if(empty($args)){
					...
					if($args[0] == "war"){
						...
						if(!$this->plugin->factionExists($args[1])){
							$sender->sendMessage($this->plugin->formatMessage("Faction dosent exist!"));
							return true;
						}
							...
							foreach($this->plugin->getServer()->getOnlinePlayers() as $p){
								if($this->plugin->getPlayerFaction($p->getName()) == $factionName){
									if($this->plugin->getLeader($factionName) == $p->getName()){
										$p->sendMessage("$sFaction wants to start war, '/f war $sFaction' to start!");
					...
					if(strtolower($args[0]) == 'info'){
						if(isset($args[1])){
							...
							$sender->sendMessage(TF::GOLD . TF::ITALIC . "-------INFORMATION-------".TF::RESET);
							$sender->sendMessage(TF::GOLD . TF::ITALIC . "|[Faction]| : " . TF::GREEN . "$faction".TF::RESET);
							$sender->sendMessage(TF::GOLD . TF::ITALIC . "|(Leader)| : " . TF::YELLOW . "$leader".TF::RESET);
							$sender->sendMessage(TF::GOLD . TF::ITALIC . "|^Players^| : " . TF::LIGHT_PURPLE . "$numPlayers".TF::RESET);
							$sender->sendMessage(TF::GOLD . TF::ITALIC . "|&Strength&| : " . TF::RED . "$power" . " STR".TF::RESET);
							$sender->sendMessage(TF::GOLD . TF::ITALIC . "|*Description*| : " . TF::AQUA . TF::UNDERLINE . "$message".TF::RESET);
							$sender->sendMessage(TF::GOLD . TF::ITALIC . "-------INFORMATION-------".TF::RESET);
						} else {
				...
				if(strtolower($args[0]) == "help"){
					...
					if($args[1] == 2){
						$sender->sendMessage(TF::GOLD . "____________.[§2Factions Help (2/7)§6]._____________" . TF::RED . "\n§b/f home§e Go to F home\n§b/f help <page>§e Look at Faction Commands\n§b/f who <faction>§e See Factions Stats\n§b/f info <faction>\n§b/f invite <player>§e To Invite Some To Your Faction\n§b/f kick <player>§e Kick Someone From Your Faction\n§b/f leader <player>§e To give Up Leader To a Faction Member\n§b/f leave§e To Leave a Faction");
						return true;
					} 
					if($args[1] == 3){
						$sender->sendMessage(TF::GOLD . "____________.[§2Factions Help (3/7)§6]._____________" . TF::RED . "\n§b/f sethome§e To Set a Faction Home\n§b/f unclaim§e To Unclaim Land\n§b/f unsethome§e To Unset F home\n§b/f ourmembers§e - {Members + Statuses}\n§b/f ourofficers§e - {Officers + Statuses}\n§b/f ourleader§e - {Leader + Status}\n§b/f allies§e - {The allies of your faction");
						return true;
					} 
					if($args[1] == 4){
						$sender->sendMessage(TF::GOLD . "____________.[§2Factions Help (4/7)§6]._____________" . TF::RED . "\n§b/f desc\n§b/f promote <player>§e To Promote a Member to a Officer\n§b/f ally <faction>§e To Ally a Faction\n§b/f unally <faction>§e To UnAlly a Faction\n§b/f allyaccept §e[Accept a request for alliance]\n§b/f allydeny §e[Deny a request for alliance]\n§b/f allies <faction>§e - {The allies of your chosen faction}");
						return true;
					} 
					if($args[1] == 5){
						$sender->sendMessage(TF::GOLD . "____________.[§2Factions Help (5/7)§6]._____________" . TF::RED . "\n§b/f membersof <faction>§e To See Members of a Faction\n§b/f officersof <faction>§e To see Officers Of a Faction\n§b/f leaderof <faction>§e To see The Leader Of a Faction\n§b/f say §e<send message to everyone in your faction>\n§b/f pf <player> §eSee A plaers List Of Top 10 Factions");
						return true;
					}
					if($args[1] == 6){
						$sender->sendMessage(TF::GOLD . "____________.[§2Factions Help (6/7)§6]._____________" . TF::RED . "\n§b/f balance §e[See faction balance]\n\n§b/f donate <amount> §e[Give money to your faction]\n\n§b/f withdraw <amount> §e[Withdraw from your faction account]");
						return true;
					}else{
						$sender->sendMessage(TF::GOLD . "____________.[§2Factions Help (7/7)§6]._____________" . TF::RED . "\n§b/f map §e[Essential base mapping tool]\n\n§b/f <c|ac> §e[Toggle faction chat/ally chat on/off]");
						return true;
					}
				}
              ...
              $this->plugin->subtractFactionPower($faction,$this->plugin->prefs->get("PowerGainedPerPlayerInFaction"));
                    ...
                    if(strtolower($args[0] == "say")){
                        if(!isset($args[1])){
                            $sender->sendMessage($this->plugin->formatMessage("Usage: /f say <message>"));
                            return true;
                        }
                        if(!($this->plugin->isInFaction($player))){
                        ...
                        for($i=0;$resultArr = $result->fetchArray(SQLITE3_ASSOC);$i=$i+1){
                            ...
                            if($p instanceof Player){
                                $p->sendMessage(TF::ITALIC.TF::RED."<FM>".TF::AQUA." <$rank$f> ".TF::GREEN."<$player> ".": ".TF::RESET);
                                $p->sendMessage(TF::ITALIC.TF::DARK_AQUA.$message.TF::RESET);
                    ...
                    if(strtolower($args[0] == "allyaccept")){
                        if(!$this->plugin->isInFaction($player)){
							$sender->sendMessage($this->plugin->formatMessage("You must be in a faction to do this."));
                            return true;
						}
					...
					if(strtolower($args[0] == 'about')){
						$sender->sendMessage(TF::GREEN . "§l§a[Original] §r§aFactionsPRO By Thethered_§r\n§l§6[Modded] §r§6FactionsPRO Modded By Potatoe Like Never Before!");
					}
					////////////////////////////// CHAT ////////////////////////////////
